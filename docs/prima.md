...
// --- PASSWORD ---
const passwordCorretta = 'prima2026'
...
const verifica = () => {
  if (passwordInserita.value === passwordCorretta) {
    ...
    errore.value = false
    sessionStorage.setItem('sblocco_1', 'ok') // Chiave univoca per la prima
  } else {
    errore.value = true
  }
}
...
onMounted(() => {
  if (sessionStorage.getItem('sblocco_1') === 'ok') {
    sbloccato.value = true
  }
})

// FILTRO: Solo articoli con class_target = '1'
const classPosts = posts.filter(post => post.class_target === '1')
</script>
...
<div v-else class="content animated-fade">
  <p class="welcome-msg">✅ Accesso effettuato. Ecco i materiali per la 1:</p>
  <div v-if="classPosts.length === 0" class="empty-msg"><p>Non ci sono ancora materiali caricati specificamente per la classe 1.</p><small>(Assicurati di aver aggiunto <code>class_target: "1"</code> nel frontmatter degli articoli)</small></div>
  <div class="blog-list">
    <div v-for="post in classPosts" :key="post.url" class="post-item">
      <a :href="post.url" class="post-link">
        ...
        <div class="post-content">
          ...
          <p class="post-meta"><span class="badge-class">Classe 1</span> {{ new Date(post.date).toLocaleDateString('it-IT') }}</p>
        </div>
      </a>
    </div>
  </div>
