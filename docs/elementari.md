---
layout: doc 
title: Area Riservata elementari
---

🔒 Area Riservata Classe elementari
...
// --- PASSWORD ---
const passwordCorretta = 'elementari2026'
...
const verifica = () => {
  if (passwordInserita.value === passwordCorretta) {
    ...
    errore.value = false
    sessionStorage.setItem('sblocco_elementari', 'ok')
  } else {
    errore.value = true
  }
}
...
onMounted(() => {
  if (sessionStorage.getItem('sblocco_elementari') === 'ok') {
    sbloccato.value = true
  }
})
...
<div v-if="!sbloccato" class="lock-screen">
  <div class="lock-box">
    <h3>Area Protetta</h3>
    <p>Questa pagina è riservata agli studenti delle elementari.</p>
...
<div v-else class="content animated-fade">
  <p class="welcome-msg">✅ Accesso effettuato. Ecco i materiali per le elementari:</p>
  <div v-if="classPosts.length === 0" class="empty-msg"><p>Non ci sono ancora materiali per le elementari.</p></div>
  <div class="blog-list">
    <div v-for="post in classPosts" :key="post.url" class="post-item">
      <a :href="post.url" class="post-link">
        ...
        <div class="post-content">
          ...
          <p class="post-meta"><span class="badge-class">Classe elementari</span> {{ new Date(post.date).toLocaleDateString('it-IT') }}</p>
        </div>
      </a>
    </div>
  </div>
