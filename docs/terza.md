...
const verifica = () => {
  if (passwordInserita.value === passwordCorretta) {
    ...
    errore.value = false
    sessionStorage.setItem('sblocco_3', 'ok') // Chiave univoca per la terza
  } else {
    errore.value = true
  }
}
...
// FILTRO: Solo articoli con class_target = '3'
const classPosts = posts.filter(post => post.class_target === '3')
</script>
...
<div v-else class="content animated-fade">
  ...
  <div v-if="classPosts.length === 0" class="empty-msg"><p>Non ci sono ancora materiali caricati specificamente per la classe 3.</p><small>(Assicurati di aver aggiunto <code>class_target: "3"</code> nel frontmatter degli articoli)</small></div>
  <div class="blog-list">
